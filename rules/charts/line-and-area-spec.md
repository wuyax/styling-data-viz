...
* **默认隐藏数据点**：为了保持高数据墨水比 (Data-Ink Ratio)，折线序列默认配置 **`showSymbol: false`**。
* **悬停聚焦显示**：仅在鼠标 Hover 悬停时通过 Tooltip 和高亮状态触发节点 Marker，维持折线与深色背景的干练视觉。

### 3. 渐变云雾面积填充 (Gradient Cloud Area Spec)
* **云雾衰减渐变**：面积图使用 `areaStyle` 叠加 `LinearGradient` 线性渐变（顶部发光高亮透明度 25%~40%，向下平滑衰减至 `0%` 完全透明）。
* **透光半透明填充**：通过渐变透明度控制，保持背景毛玻璃卡片与坐标网格通透呈现。
