...
description: Use when building, styling, reviewing, or auditing dark-mode ECharts data visualizations, screens, cockpits, guofeng (国风), or huanzi (幻紫) cyber neon dashboards.
---
...
无缝融入大屏毛玻璃卡片 ➔ 载入 [`rules/universal/baseline-spec.md`](rules/universal/baseline-spec.md)：
- **透明容器**：`backgroundColor: 'transparent'`, `grid.containLabel = true`
- **低噪轴线**：隐藏 Y 轴线，横向网格设为 `rgba(205, 225, 248, 0.05)` 超淡虚线，隐藏纵向网格
- **等宽数值与字体**：关键数值强制配置 `'D-DIN, DINPro-Medium, monospace'` 与 `font-feature-settings: "tnum"` ➔ 载入 [`rules/universal/typography-and-numbers.md`](rules/universal/typography-and-numbers.md)
...
- **自适应响应式**：绑定 `ResizeObserver` 防抖 resize ➔ 载入 [`rules/universal/layout-and-responsive.md`](rules/universal/layout-and-responsive.md)
...
- **仪表盘 / 水球图** ➔ [`rules/charts/gauge-spec.md`](rules/charts/gauge-spec.md)：无针化进度条 `pointer: { show: false }`
- **地图 / 迁徙飞线** ➔ [`rules/charts/map-and-flyline-spec.md`](rules/charts/map-and-flyline-spec.md)：发光陆块 + 低频巡航飞线

### Step 4. 自检与 Completion Criteria
输出 ECharts 配置选项 (option) 之前，必须对以下 6 项标准逐一核对：
1. [ ] `backgroundColor` 是否已显式设为 `'transparent'`？
2. [ ] 是否已配置 `LinearGradient` 渐变填充（严禁单色平涂大面积色块）？
...
6. [ ] 是否已查阅并修正 [`rules/universal/anti-patterns.md`](rules/universal/anti-patterns.md) 中的常见反模式？

---

## 目录索引 (Directory Index)

- **通用规范**：[`baseline-spec.md`](rules/universal/baseline-spec.md) | [`anti-patterns.md`](rules/universal/anti-patterns.md) | [`typography-and-numbers.md`](rules/universal/typography-and-numbers.md) | [`stripe-texture-and-gradients.md`](rules/universal/stripe-texture-and-gradients.md) | [`animation-and-rhythm.md`](rules/universal/animation-and-rhythm.md) | [`layout-and-responsive.md`](rules/universal/layout-and-responsive.md)
- **图表特有规范**：[`bar-spec.md`](rules/charts/bar-spec.md) | [`pie-and-donut-spec.md`](rules/charts/pie-and-donut-spec.md) | [`line-and-area-spec.md`](rules/charts/line-and-area-spec.md) | [`combo-dual-axis-spec.md`](rules/charts/combo-dual-axis-spec.md) | [`gauge-spec.md`](rules/charts/gauge-spec.md) | [`map-and-flyline-spec.md`](rules/charts/map-and-flyline-spec.md)
